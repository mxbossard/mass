...
func read[T any](collection, resource string) (T, error) {
	var v T
	err := db.Read(collection, resource, &v)
	return v, err
}

func write[T any](collection, resource string, v T) error {
	err := db.Write(collection, resource, v)
	return err
}

func readAll[T any](collection string) ([]T, error) {
	var items []T
	records, err := db.ReadAll(collection)
	if err != nil {
		return nil, err
...
func listResourcesCollections(namespace string) (collections []string, err error) {
	...
	allCollections, err := readAll[string](collectionsCollection)
	if err != nil {
		return nil, err
	}
...
func listMappedResources(namespace, kind, name string) ([]map[string]any, error) {
	...
	if kind == "" {
		...
		for _, collection := range collections {
			allRes, err := readAll[map[string]any](collection)
			if err != nil {
				return nil, err
			}
	...
	records, err := readAll[map[string]any](kindCollection)
	if err != nil {
		return nil, err
	}
